...
export default function CreateCvPage() {
  ...
  return <section className="form-page"><h1>Start an application</h1><p className="lede">Step {step} of 4 · Paste a job post, review evidence, then generate a targeted application.</p>
    <form className="create-record-form" onSubmit={(event) => { event.preventDefault(); setMessage(draft.text.trim() ? "Draft saved in this browser session." : "Paste the job description first."); }}>
      ...
      <button className="button button-primary" type="button" onClick={() => void startWorkflow()} disabled={!draft.text.trim() || !draft.company || !draft.role}>Build application</button>{step === 2 && <div className="review-card"><h2>Review evidence</h2><p>Check the matched source evidence before drafting.</p><button className="button button-primary" type="button" onClick={() => void resumeWorkflow("approve")}>Approve evidence</button><button className="button button-secondary" type="button" onClick={() => void resumeWorkflow("reject")}>Reject and revise</button></div>}{step === 3 && <p role="status">Generation complete. Final review is ready.</p>}{message && <p role="status">{message}</p>}
    </form>
  </section>;
}
